...
// Shape used by applyStylesFormat/countNodes/topSections (mirrors their AnyRec param)
// with an index signature so it stays assignable to Record<string, unknown>.
interface TestNode {
  [key: string]: unknown
  id?: string
  name?: string
...
interface BoundsStyles {
  bounds: { x: number; width: number }
  styles: { opacity: number; paddingLeft: number }
}
...
{
  const out = transformNumbers({ bounds: { x: 1.4, width: 2.6 }, styles: { opacity: 0.5, paddingLeft: 3.7 } }, true, 2) as BoundsStyles
  ...
  const p = transformNumbers({ x: 1.23456, o: 0.98765 }, false, 2) as { x: number; o: number }
...
{
  const node: TestNode = { styles: { opacity: 1, blendMode: 'NORMAL', paddingLeft: 0, fills: [], color: '#fff', locked: false } }
  applyStylesFormat(node, 'compact')
...
// applyStylesFormat: classes extracts repeated styles
{
  ...
  const root: TestNode = { styles: { ...shared }, children: [{ styles: { ...shared } }, { styles: { unique: 1 } }] }
...
{
  const root: TestNode = { id: 'r', name: 'root', children: [{ id: 'a', name: 'A', bounds: { y: 10, height: 50 }, children: [{ id: 'a1' }] }, { id: 'b', name: 'B' }] }
