...
export async function handleGetNodeVariableBindings(params: Record<string, unknown>): Promise<unknown> {
  ...
  return {
    boundVariables: ('boundVariables' in node) ? (node as SceneNode).boundVariables : undefined,
    inferredVariables: ('inferredVariables' in node) ? (node as SceneNode).inferredVariables : undefined,
    resolvedVariableModes: ('resolvedVariableModes' in node) ? (node as SceneNode).resolvedVariableModes : undefined,
  }
}
